...
-- ============================================================
-- 新增：导入配置（从 JSON 数据直接应用，不写入文件）
-- ============================================================
function SaveManager:ImportConfig(ConfigData: string): (boolean, string?)
    if IsStringEmpty(ConfigData) then
        return false, "未提供配置数据"
    end

    local SuccessDecode, Decoded = pcall(HttpService.JSONDecode, HttpService, ConfigData)
    if not SuccessDecode or typeof(Decoded) ~= "table" or typeof(Decoded.objects) ~= "table" then
        return false, "解析配置数据失败"
    end
...
function SaveManager:BuildConfigSection(Tab: any, IconName: string)
    ...
    local ConfigurationBox = Tab:AddRightGroupbox("配置管理", nil, nil, false)
    ...
    ConfigurationBox:AddButton({
        ...
        Func = function()
            ...
            if not setclipboard then
                SaveManager.Library:Notify("当前环境不支持复制到剪贴板")
                return
            end

            local Success, Data = SaveManager:ExportConfig(ConfigName)
            if not Success then
                SaveManager.Library:Notify(string.format("导出配置 %q 失败：%s", ConfigName, tostring(Data)))
                return
            end

            setclipboard(Data)
            SaveManager.Library:Notify(string.format("成功导出配置 %q 到剪贴板", ConfigName))
        end
    })
    ...
    -- ============================================================
    -- 新增：导入配置（粘贴 JSON 后直接应用）
    -- ============================================================
    ConfigurationBox:AddInput("SaveManager_ImportData", {
        Text = "导入配置数据",
        Placeholder = "粘贴从剪贴板复制的配置 JSON..."
    })
    ...
    ConfigurationBox:AddButton({
        ...
        Func = function()
            ...
            if IsStringEmpty(ConfigData) then
                SaveManager.Library:Notify("请先粘贴要导入的配置数据")
                return
            end
    ...
    RefreshAutoloadConfigLabel()
    SaveManager:SetIgnoreIndexes({ "SaveManager_ConfigList", "SaveManager_ConfigName", "SaveManager_ImportData" })
